...
def test_qmoney_add_two_values(price, tax):
    result = (price + tax).observe().to_decimal()
...
def test_qmoney_chained_operations(price, tax):
    result = (price * 3 + tax).observe().to_decimal()
...
def test_qmoney_complex_expression():
    ...
    result = (a * 3 - b + QMoney(Decimal("0.25"))).observe().to_decimal()
